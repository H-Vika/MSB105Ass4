...
### Spania
#### Oppgave 25
Lag et linje-plot som viser utviklingen i Gini-koeffisientene for NUTS2 regionene i Spania.
...
### Frankrike
#### Oppgave 29
Vis utviklingen i Gini-koeffisient for NUTS1 regionene i Frankrike i et line-plot. Plottet
vil se slik ut.
...
#### Oppgave 36
Bruk glance() funksjonen fra broom pakken og «map» denne på modell variabelen for å generere «model summary». Legg resultatet i en variabel mod_sum.
...
mod_coeff_tbl <- list_rbind(map(NUTS2_diff$modell, coeff))
stopifnot(nrow(mod_coeff_tbl) == nrow(NUTS2_diff))


glance_safe <- safely(glance, otherwise = tibble(
  r.squared = NA_real_, adj.r.squared = NA_real_, sigma = NA_real_,
  statistic = NA_real_, p.value = NA_real_, df = NA_real_, logLik = NA_real_,
  AIC = NA_real_, BIC = NA_real_, deviance = NA_real_, df.residual = NA_real_, nobs = NA_real_
))

mod_sum_list <- map(NUTS2_diff$modell, ~ glance_safe(.x)$result)
mod_sum_tbl  <- list_rbind(mod_sum_list)
stopifnot(nrow(mod_sum_tbl) == nrow(NUTS2_diff))


NUTS2_diff <- bind_cols(NUTS2_diff, mod_coeff_tbl, mod_sum_tbl) |>
  mutate(
    r2   = r.squared,
    r2_a = `adj.r.squared`
  )
...
NUTS2_diff |>
  arrange(desc(r2)) |>
  select(nc_name, n2, NUTS2_diff, modell, `(Intercept)`, r2) |>
  head(3)
